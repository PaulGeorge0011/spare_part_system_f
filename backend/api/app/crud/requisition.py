...
def requisition_spare_part(
    ...
    """
    领用备件：扣减修复件库存并记录领用日志。
    返回领用结果摘要，失败返回 None（调用方需先校验库存等）。
    """
    item = db.query(SparePart).filter(SparePart.id == spare_part_id).first()
    if not item:
        return None
    ...
    # 只传入模型上存在的列，兼容工厂未更新 models/requisition_log.py 的旧部署
    log_attrs = {
    ...
    model_keys = {c.key for c in RequisitionLog.__table__.columns}
    log_kw = {k: v for k, v in log_attrs.items() if k in model_keys}
    log = RequisitionLog(**log_kw)
    db.add(log)
    db.commit()
    db.refresh(item)
    return {
...
def get_unreturned_quantity(db: Session, spare_part_id: int, operator_name: str) -> int:
    """计算当前用户对指定备件的未归还数量 = 已领用总量 - 已归还总量。"""
    name = (operator_name or "").strip()
    if not name:
        return 0
    requisitioned = db.query(func.coalesce(func.sum(RequisitionLog.quantity), 0)).filter(
        RequisitionLog.spare_part_id == spare_part_id,
        (RequisitionLog.operator_name == name) | (RequisitionLog.requisitioner_name == name),
    ).scalar() or 0
    returned = db.query(func.coalesce(func.sum(RequisitionReturnLog.quantity), 0)).filter(
        RequisitionReturnLog.spare_part_id == spare_part_id,
        (RequisitionReturnLog.operator_name == name) | (RequisitionReturnLog.requisitioner_name == name),
    ).scalar() or 0
...
def return_spare_part(
    ...
    unreturned = get_unreturned_quantity(db, spare_part_id, name)
...
def get_recent_requisition_logs(
    db: Session,
    operator_name: str,
    limit: int = 10,
) -> List[Dict[str, Any]]:
    """当前用户的最近领用记录（电气），按领用时间倒序。附带该备件的未归还数量。"""
    if not operator_name or not str(operator_name).strip():
        return []
    name = str(operator_name).strip()
    logs = (
        db.query(RequisitionLog)
        .filter(
            (RequisitionLog.operator_name == name) | (RequisitionLog.requisitioner_name == name)
        )
        .order_by(desc(RequisitionLog.requisition_at))
        .limit(limit)
        .all()
    )
    out = []
    for log in logs:
        part = db.query(SparePart).filter(SparePart.id == log.spare_part_id).first()
        unreturned = get_unreturned_quantity(db, log.spare_part_id, name)
        out.append({
            "id": log.id,
