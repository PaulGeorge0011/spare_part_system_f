# app/models/mechanical_spare_part_image.py
...
class MechanicalSparePartImage(Base):
    __tablename__ = "mechanical_spare_part_images"
    ...
    mechanical_spare_part_id = Column(
        Integer,
        ForeignKey("mechanical_spare_parts.id", ondelete="SET NULL"),
        nullable=True,
        comment="关联机械备件ID",
    )
    mechanical_spare_part = relationship("MechanicalSparePart", back_populates="images")
    ...
    def __repr__(self):
        return f"<MechanicalSparePartImage(id={self.id}, filename='{self.filename}')>"
