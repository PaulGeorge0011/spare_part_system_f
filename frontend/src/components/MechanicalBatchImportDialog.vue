...
function getOptimizedRange(worksheet: XLSX.WorkSheet): string | null {
  try {
    ...
    if (totalCells < 1000000) {
      console.log(`[MechanicalBatchImport] 工作表范围正常: ${ref}, 单元格数: ${totalCells}`)
      return null
    }
    
    console.warn(`[MechanicalBatchImport] 工作表范围过大: ${ref}, 单元格数: ${totalCells}, 开始优化...`)
    ...
    if (maxRow === -1 || maxCol === -1) {
      console.warn('[MechanicalBatchImport] 未找到有效数据，使用原始范围')
      return null
    }
    ...
    console.log(`[MechanicalBatchImport] 优化后范围: ${newRange}, 单元格数: ${newTotalCells}`)
    
    return newRange
  } catch (e) {
    console.warn('[MechanicalBatchImport] 优化范围失败:', e)
    return null
  }
}
...
async function startImport() {
  ...
  try {
    ...
    // 逐条调用 createMechanicalSparePartWithImages
    let successCount = 0
    let failedCount = 0
    let skippedCount = 0
    ...
    for (let idx = 0; idx < sorted.length; idx++) {
      ...
      // material_code 需符合后端要求（3–50 位字母数字下划线短横线），否则用 row_N
      ...
      // 后端要求 mes_stock、physical_stock 为数字，空字符串会校验失败，需规范为数字
