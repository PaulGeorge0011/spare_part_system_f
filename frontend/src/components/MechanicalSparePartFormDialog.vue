  ...
  >
    ...
    >
      ...
      <el-row :gutter="isMobile ? 0 : 20">
        ...
        <el-col :xs="24" :sm="12">
          <el-form-item label="修附件库存" prop="physical_stock">
            <el-input-number
      ...
      <el-row :gutter="isMobile ? 0 : 20">
        ...
        <el-col :xs="24" :sm="12">
          <el-form-item label="保管人">
            <el-input v-model="formData.custodian" placeholder="保管人" clearable maxlength="100" show-word-limit />
          </el-form-item>
        </el-col>
      </el-row>

      <!-- 机械设备专属：图号、来源说明、技术鉴定、处置方式 -->
      <el-row :gutter="isMobile ? 0 : 20">
        <el-col :xs="24" :sm="12">
          <el-form-item label="图号">
            <el-input v-model="formData.drawing_no" placeholder="图号" clearable maxlength="100" show-word-limit />
          </el-form-item>
        </el-col>
        <el-col :xs="24" :sm="12">
          <el-form-item label="处置方式">
            <el-input v-model="formData.disposal_method" placeholder="处置方式" clearable maxlength="200" show-word-limit />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="isMobile ? 0 : 20">
        <el-col :span="24">
          <el-form-item label="来源说明">
            <el-input v-model="formData.source_description" type="textarea" :rows="2" placeholder="来源说明" clearable maxlength="500" show-word-limit />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="isMobile ? 0 : 20">
        <el-col :span="24">
          <el-form-item label="技术鉴定">
            <el-input v-model="formData.technical_appraisal" type="textarea" :rows="2" placeholder="技术鉴定" clearable maxlength="500" show-word-limit />
          </el-form-item>
        </el-col>
      </el-row>
...
import type { MechanicalSparePart, MechanicalSparePartCreate, MechanicalSparePartUpdate } from '@/types/mechanicalSparePart'
import {
  getTempImage,
  getTempImagesBatch
...
import {
  mechanicalSparePartApi,
  createMechanicalSparePartWithImages,
  updateMechanicalSparePartWithImages,
} from '@/api/mechanicalSparePart'
import { broadcastMechanicalSparePartDataChanged } from '@/composables/useMechanicalSparePartDataChanged'

// 定义组件Props
interface Props {
  modelValue: boolean
  formData?: Partial<MechanicalSparePart>
  mode?: 'create' | 'edit'
}
...
const emit = defineEmits<{
  ...
  'success': [data: MechanicalSparePart]
  'cancel': []
}>()
...
// 表单数据（含机械专属：图号、保管人、来源说明、技术鉴定、处置方式）
const formData = reactive<MechanicalSparePartCreate & MechanicalSparePartUpdate & { id?: number }>({
  location_code: '',
  mes_material_code: '',
  mes_material_desc: '',
...
const formRules: FormRules = {
  ...
  physical_stock: [
    { required: true, message: '请输入修附件库存', trigger: 'blur' },
    { type: 'number', min: 0, message: '库存不能为负数', trigger: 'blur' },
  ],
  unit: [
...
const formTitle = computed(() => {
  return isEditMode.value ? '编辑机械设备' : '新增机械设备'
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const submitData: any = {
      ...
      technical_appraisal: formData.technical_appraisal,
      disposal_method: formData.disposal_method,
    }
    
    let result: MechanicalSparePart
    
    if (isEditMode.value && formData.id) {
      result = await updateMechanicalSparePartWithImages(
        formData.id,
        submitData,
        allUploadIds,
        imagesToDelete.value
      ...
      result = await createMechanicalSparePartWithImages(
        submitData,
        allUploadIds
      )
    }
    ...
    ElMessage.success(isEditMode.value ? '机械设备更新成功' : '机械设备创建成功')
...
const loadExistingImages = async () => {
  ...
  try {
    ...
    const detail = await mechanicalSparePartApi.getDetailWithImages(formData.id!) as any
    ...
    if ((!formData.physical_image_url || !formData.physical_image_url2) && existingImages.value.length > 0) {
      try {
        await request.post(`/mechanical-spare-parts/${formData.id!}/sync-images`)
        const updated = await mechanicalSparePartApi.getDetailWithImages(formData.id!) as any
